...
A ready-made folded checkpoint is published as [open-machine/gemma-4-E2B-FlashNorm](https://huggingface.co/open-machine/gemma-4-E2B-FlashNorm) (gains of `input_layernorm` and `pre_feedforward_layernorm` folded, loads in stock Transformers); `cancel_pre_attention_norms` applies to it directly. A strict variant with the folded norm tensors removed and the pre-attention RMSNorm cancelled in the modeling code (`flashNorm_modeling_gemma4.py`, trust_remote_code) is at [open-machine/gemma-4-E2B-FlashNorm-strict](https://huggingface.co/open-machine/gemma-4-E2B-FlashNorm-strict); its outputs are bitwise identical (bf16) to the compat checkpoint with runtime cancellation.
