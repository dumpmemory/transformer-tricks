...
We additionally propose a simplified MLA architecture that eliminates the separate RoPE projection cache, reducing both parameter count and long-sequence KV-cache overhead.
...
We validate MatShrink's core claim of mathematical equivalence empirically on four models spanning both attention variants covered by the paper and both regimes of head dimension: SmolLM2-135M \citep{smollm}, a 30-layer decoder-only transformer with grouped-query attention ($n_\text{heads} = 9$, $n_\text{KV} = 3$, $d_\text{head} = 64$); GPT-2-124M \citep{gpt2}, a 12-layer multi-head attention transformer ($n_\text{heads} = 12$, $d_\text{head} = 64$); and Gemma-3 \citep{gemma3} at the 1B (text-only) and 4B (multimodal, text-backbone-only) sizes, both with $d_\text{head} = 256$, the regime where MatShrink's per-head $r^2$ savings accumulate most (Table \ref{tab1}). All experiments run on a single NVIDIA A100 GPU and evaluate perplexity on the full WikiText-2 raw test split using non-overlapping context-length windows.
