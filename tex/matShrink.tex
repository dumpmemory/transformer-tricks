...
See \citep{tricks} for code and more transformer tricks.
\end{abstract}
...
\begin{figure}[h!] \centering  % the [h!] tries to place the picture right here
  ...
  % TODO: \caption{Proposition~\ref{prop:matshrink} illustrated: two back-to-back matrices $W_A \in \mathbb{R}^{d \times r}$ and $W_B \in \mathbb{R}^{r \times e}$ (left) are reparametrized so that the leading $r \times r$ block of $W_B$ becomes the identity, saving $r^2$ stored parameters (right). $d > r$ and $e > r$.}
...
\begin{enumerate}[topsep=2pt, itemsep=1pt]
  ...
  \item We show that the attention dot-product is preserved under MatShrink applied to Q-K projections, even though the individual queries and keys change, and derive the savings for partial-RoPE models (Section~\ref{sec:mha}).
  ...
  \item We identify the bf16 numerical instability, diagnose its root cause as ill-conditioned transformation matrices, and show that pivoted QR basis selection is optimal among all column-permutation strategies (Section~\ref{sec:precision}).
...
%A full-rank $W_B$ guarantees an invertible $r \times r$ sub-block exists. Since the set of rank-deficient matrices has Lebesgue measure zero in $\mathbb{R}^{r \times e}$ \citep{invertible}, this condition holds generically. In practice it is enforced by column permutation via pivoted QR (Section~\ref{sec:precision}).
%\end{remark}

\begin{remark}[Dual formulation]
\label{rem:dual}
An equivalent $r^2$ reduction is obtained by shrinking $W_A$ instead. Partition $W_A = [W_{A1};\, W_{A2}]$ with $W_{A1} \in \mathbb{R}^{r \times r}$ invertible, $W_{A2} \in \mathbb{R}^{(d-r) \times r}$. Then $W_A W_B = [I_r;\, W_{A2}^*] W_A^{**}$ where $W_A^{**} = W_{A1} W_B$ and $W_{A2}^* = W_{A2} W_{A1}^{-1}$. This is the natural choice when it is preferable to preserve the structure of $W_B$ (Fig.~\ref{fig2}).
\end{remark}
...
\begin{figure}[h!] \centering
  ...
  \caption{MatShrink (Proposition~\ref{prop:matshrink}) applied to the V-O projection pair of a single MHA head: (a) original; (b) compressed, eliminating $r^2 = d_k^2$ weights from $W_O$.}
  % TODO: \caption{MatShrink for V and O projections of a single attention head: (a) original V and O projections; (b) equivalent implementation using MatShrink, which eliminates $r^2$ weights from $W_O$.}
\label{fig3} \end{figure}

\paragraph{MHA V-O savings.}\label{prop:mha-vo}
For MHA with $h$ heads and model dimension $d = d_\text{model}$, each head has dimension $r = d_k = d/h$. Applying MatShrink to every head's V-O pair eliminates $r^2 = d^2/h^2$ weights per head and $d^2/h$ weights per layer in total---a fraction $1/(2h)$ of the $2d^2$ total V-O parameters. This follows directly from the weight savings formula with $e = d$ and $r = d/h$: savings per head $= r^2 = d^2/h^2$; summing over $h$ heads gives $d^2/h$.

Table~\ref{tab1} instantiates the MHA V-O savings formula on representative models.
...
Figure~\ref{fig4} shows the dual V-O formulation (Remark~\ref{rem:dual}), eliminating weights from $W_V$ instead.
\begin{figure}[h!] \centering
  \includegraphics[scale=0.88]{../doc/fig/matShrink_fig4.pdf}
  \caption{Dual V-O MatShrink (Remark~\ref{rem:dual}): $r^2$ weights eliminated from $W_V$ rather than $W_O$.}
  % TODO \caption{Alternative MatShrink implementation for V and O projections of a single attention head: (a) original V and O projections; (b) equivalent implementation using the alternative MatShrink scheme from Fig. \ref{fig2}, which eliminates $r^2$ weights from $W_V$.}
\label{fig4} \end{figure}

\paragraph{Q-K projections.}
For models without RoPE (e.g., Whisper \cite{whisper}, T5 \cite{T5}), the query and key projections $W_{Q,i}, W_{K,i}$ form a back-to-back pair whose composition determines the attention logits.

\begin{figure}[h!] \centering
  \includegraphics[scale=0.88]{../doc/fig/matShrink_fig5.pdf}
  \caption{MatShrink for Q-K projections (Section~\ref{sec:mha}): the attention dot-product $p$ is preserved despite $Q^* \neq Q$ and $K^* \neq K$.}
  % TODO \caption{MatShrink for Q and K projections of a single attention head. Left: original Q and K projections and their dot-product $p$. Right: equivalent implementation using MatShrink, which eliminates $r^2$ weights from $W_Q$.}
\label{fig5} \end{figure}

\paragraph{Q-K dot-product invariance under MatShrink.}\label{prop:qk}
Let $W_Q, W_K \in \mathbb{R}^{d \times r}$ be query and key projection matrices. For any invertible $T \in \mathbb{R}^{r \times r}$, define $W_Q^* = W_Q T$ and $W_K^* = W_K T^{-\top}$. Then the attention dot-product is preserved:
\[
  (x W_Q^*)(x W_K^*)^\top = x W_Q T \cdot T^{-1} W_K^\top x^\top = (x W_Q)(x W_K)^\top.
\]
Setting $T = W_{Q1}$ (the leading $r \times r$ block of $W_Q$, assumed invertible) yields MatShrink: $W_Q^*$ has an identity leading block, eliminating $r^2$ weights from $W_Q$.

\paragraph{Partial RoPE.}
Many modern models apply RoPE to a fraction $\alpha \in (0,1)$ of $d_k$ (typically $\alpha = 1/2$). The dot-product invariance result above applies to the non-RoPE portion of size $(1-\alpha)d_k$, yielding per-head savings of $(1-\alpha)^2 d_k^2$. For $\alpha = 1/2$: savings $= d_k^2/4$ per head.
...
\subsection{The bf16 Problem}
MatShrink stores $W_A^* = W_A M$ and $W_{B2}^* = M^{-1} W_{B2}$. Under fp32, error is $\mathcal{O}(10^{-14})$ (negligible). Under bf16, the 7-bit mantissa causes rounding of $W_A^*$ and $W_{B2}^*$ proportional to $\cond(M)$: with the naive $M = W_B[:r,:r]$ choice, condition numbers reach $\cond(M) \sim 10^5$, causing perplexity regressions of up to $+25$ absolute points (Table~\ref{tab:matshrink-bf16}).

\subsection{Pivoted QR Is Optimal}
As noted in Proposition~\ref{prop:matshrink}, the sub-block $M$ can be any invertible $r \times r$ sub-block of $W_B$; different choices yield the same function but different numerical properties of the stored weights. We seek the choice that minimizes $\cond(M)$.

\paragraph{Pivoted QR minimizes worst-case condition number.}\label{prop:pqr}
Among all $\binom{e}{r}$ ways to choose $r$ columns of $W_B$ as the sub-block $M$, the pivoted QR decomposition
\[
  (Q, R, P) \gets \mathrm{pQR}(W_B), \qquad M \gets W_B[:, P[:r]]
\]
selects the $r$ columns that maximize $|\det(M)|$ subject to column norms and, by the interlacing property of singular values, yield the smallest achievable worst-case $\cond(M)$ among column-permutation strategies. This follows because pivoted QR with column pivoting greedily selects at each step the column with largest residual norm---equivalent by the Cauchy--Binet formula to maximizing the absolute value of the leading $r \times r$ minor. A larger $|\det(M)|$ implies a larger $\sigma_{\min}(M)$ (since $\sigma_{\min}(M) \geq |\det(M)| / \prod_{i=1}^{r-1} \sigma_i(M)$), and hence a smaller $\cond(M) = \sigma_{\max}(M)/\sigma_{\min}(M)$ for fixed $\sigma_{\max}(M)$.

\begin{remark}[Explicit change of basis]
The pivoted-QR choice corresponds to the change of basis $T = M \cdot M_0^{-1}$ where $M_0 = W_B[:, :r]$. Running MatShrink on $(W_A T, T^{-1} W_B)$ produces the same $(W_A^*, W_{B2}^*)$ as directly using $M$ as the sub-block. The direct approach is a one-line call: \texttt{scipy.linalg.qr(W\_B.T, pivoting=True)}.
\end{remark}
...
\section{Conclusion}
We presented MatShrink, a theoretically grounded, empirically validated framework for losslessly eliminating $r^2$ parameters from any pair of consecutive linear layers. The core result (Proposition~\ref{prop:matshrink}) is an elementary change-of-basis identity; the pivoted QR analysis (Section~\ref{sec:precision}) establishes the numerically optimal choice of sub-block. Specializations to MHA, MLA, GQA, MQA (Sections~\ref{sec:mha}--\ref{sec:gqa}) quantify savings of 8--17\% of attention parameters for MHA and $\sim$414M parameters for DeepSeek-R1/V3-scale MLA. Experiments on four models confirm exact fp32 equivalence and near-exact bf16 equivalence, with compressed checkpoints loading in production inference engines without modification.

\paragraph{Limitations and future work.}
The weight savings currently do not yield proportional throughput improvements, since identity blocks are stored and multiplied as dense tensors; realizing the compute reduction requires structured-sparse runtime support. Extending empirical validation to MLA models, evaluating composition with post-training quantization (Section~\ref{sec:precision}), integrating MatShrink-aware kernels into production engines (llama.cpp \citep{llama-cpp}, SGLang \citep{sglang}) are natural directions for future work.
