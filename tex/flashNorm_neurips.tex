...
entirely, see Fig. \ref{figB}(b). This provides a lossless recipe for converting a LayerNorm model to RMSNorm without any retraining; see Appendix~\ref{app:additional-notes} for the residual-stream completeness condition.
\begin{figure}[h!] \centering  % the [h!] tries to place the picture right here
...
One of these scaling operations can be deferred to the FFN output, saving $f - n$ multiply operations relative to Fig. \ref{fig3}(a) as shown in Fig. \ref{fig3}(b). See appendix for FFNs with ReLU, ReGLU and Bilinear GLU variants.
\begin{figure}[h!] \centering
...
GPU benchmarks confirm 33--35\% operation-level latency reduction at SmolLM2-135M scale
