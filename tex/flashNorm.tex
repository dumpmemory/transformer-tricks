...
\emph{non-parametric}~\citep{olmo2}; \citet{remove-norm2} show that LayerNorm
can be removed post-hoc from GPT-2 XL with a learned ablation. FlashNorm differs in being a closed-form algebraic restatement (exact in real arithmetic and bit-identical at runtime precision) that requires no retraining or empirical recovery, and additionally exposes a parallel-execution opportunity (Proposition~\ref{prop:commute}); Appendix~\ref{app:additional-notes} provides an extended comparison.
...
\]
where $\mathbf{g} \in \mathbb{R}^n$ are learned normalization weights.\footnote{Production implementations include a small regularizer $\varepsilon$ inside the square root to prevent division by zero; we use the unregularized form for the proofs of Propositions~\ref{prop:weightless} and~\ref{prop:commute} and treat the $\varepsilon$-regularized case in Section~\ref{sec:rms-cancel} and Appendix~\ref{app:epsilon}.}
...
Deferring the scalar to after the matrix multiplication enlarges the dynamic range of the intermediate $\mathbf{a}\mathbf{W}^*$; implementations accumulate it in a higher precision (typically FP32 behind FP16/BF16 storage). See Section~\ref{sec:limitations-discussion} and Appendix~\ref{app:additional-notes}.
\end{remark}
...
a technique already used in practice (e.g., SmoothQuant~\citep{smoothQuant}). Appendix~\ref{app:additional-notes} contains a per-row analysis of how the fold affects weight dynamic range and the choice of per-channel vs per-tensor quantization. We leave a systematic empirical study of this interaction to future work.
...
accumulate the unnormalized matmul output in FP32, which mitigates this risk in practice. Low-precision floating-point formats (notably FP8) require additional care; see Appendix~\ref{app:additional-notes} for the FP8 E4M3 caveat.
...
Under lossy inference precisions, post-hoc weight folding can produce one-token argmax divergences at tight decision points; the effect is visible at Llama-7B scale and is not specific to FlashNorm. See Appendix~\ref{app:additional-notes}.
