...
Additionally, by the scale invariance of RMS,
an RMSNorm followed by a linear layer followed by another RMSNorm allows the first
RMSNorm to be eliminated entirely---a mathematically identical simplification that removes the
pre-attention RMSNorm in models using QKV-normalization (e.g., Gemma~4) and in MLA-models
with latent normalization (e.g., DeepSeek-V2, Mistral Small 4,  and OpenMythos).
The same techniques extend to LayerNorm, Dynamic Tanh (DyT), feed-forward networks
...
\paragraph{Realized HuggingFace decode speedup.}
On \texttt{Llama-3.2-1B} at bf16, FlashNorm Proposition~\ref{prop:weightless} delivers $+12.77\%$ end-to-end in stock HuggingFace Transformers when \texttt{LlamaRMSNorm.forward} is routed through PyTorch's existing weightless C++ kernel \texttt{F.rms\_norm(x, shape, weight=None, eps)}, which skips the per-channel multiply when the weight argument is \texttt{None}; details in Appendix~\ref{app:realized-speedup}.
...
\begin{table}[h!] \centering
\caption{RMSNorm wall-clock fraction during 256-token greedy decode on \texttt{Llama-3.2-1B-Instruct}, A100, fp16 inputs, median of 20 trials. Norm time is cumulative across all per-layer RMSNorm calls.}
...
4-bit (NF4 + DQ)      & $14.5$ & $17{,}720$ & $1{,}650$ & $15.01\%$ \\
\hline
\end{tabular}
\end{table}

\paragraph{Findings.} RMSNorm occupies 24.62\% of decode wall-clock at bf16, an order of magnitude above its FLOP share, because the model runs 33 separate RMSNorm kernels per forward pass and each incurs a fixed kernel-launch latency on the order of $10\,\mu$s plus a memory-bandwidth pass over the full activation tensor independent of the small RMS arithmetic. The cumulative norm wall-clock is invariant under weight quantization (within $\pm 2\%$ across all three precisions: $1{,}640$, $1{,}675$, $1{,}650$\,ms), as BitsAndBytes does not quantize norm weights. The norm fraction is non-monotonic across precisions on this hardware ($24.62\% \to 8.19\% \to 15.01\%$) because BitsAndBytes inflates the linear path's wall-clock at low bit on A100 (\texttt{MatMul8bitLt} dequantize overhead makes 8-bit $3\times$ slower than bf16; NF4 is more efficient but still $2.7\times$ slower); the fraction shrinks at lower bit not because the norm becomes cheaper but because the BitsAndBytes linear becomes more expensive. On hardware-and-framework combinations with native low-bit kernels (e.g., \texttt{llama.cpp} GGUF, MLX 4-bit on Apple silicon, vLLM and TensorRT-LLM with int4 at scale), the invariant norm cost would translate to a monotonically growing fraction; direct measurement on those stacks is left to follow-up.

\section{Realized Speedup in HuggingFace Transformers}\label{app:realized-speedup}

We measure the realized end-to-end speedup of FlashNorm Proposition~\ref{prop:weightless} in stock HuggingFace Transformers by comparing two forward paths on the FlashNorm-folded checkpoint \texttt{open-machine/Llama-3.2-1B-FlashNorm}: (A) the default \texttt{LlamaRMSNorm.forward}, which always computes \texttt{self.weight * x\_normalized} even when \texttt{self.weight} is the all-ones tensor that FlashNorm folding leaves behind; and (B) a runtime replacement of \texttt{forward} that calls \texttt{F.rms\_norm(x, shape, weight=None, eps)}, routing into PyTorch's torch C++ kernel that branches on \texttt{weight is None} and skips the per-channel multiply. Both variants run on the same checkpoint, isolating kernel choice from folding. On NVIDIA A100 (40\,GB) at bf16, 256-token greedy decode, median of 20 trials with 3 warmup runs and eager attention, variant (A) achieves $45.00$\,tokens/s and variant (B) achieves $50.75$\,tokens/s: a relative speedup of $\mathbf{+12.77\%}$. The mechanism is Proposition~\ref{prop:weightless} composed with PyTorch's existing weightless code path; the remaining per-token RMSNorm cost (variance, rsqrt, fp32 cast) is unchanged. Production deployments can realize this speedup today by routing \texttt{LlamaRMSNorm.forward} through \texttt{F.rms\_norm} with \texttt{weight=None} on FlashNorm-folded checkpoints, with a small upstream patch in \texttt{transformers} making the dispatch automatic via a flag on \texttt{LlamaRMSNorm}. Reproducibility notebook in the \texttt{transformer-tricks} repository\footnote{\url{https://github.com/OpenMachine-ai/transformer-tricks/blob/main/notebooks/flashNorm\_hf\_a100.ipynb}}.
