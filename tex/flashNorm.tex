...
zero-loss weight folding on SmolLM2-135M, Llama-3.2-1B, and Llama-3.1-8B, and
release three public compatibility checkpoints that load unmodified in stock
...
The bottleneck is most acute in the autoregressive decode regime that dominates production inference serving. At batch size one, each transformer layer launches two small, memory-bound kernels (RMSNorm followed by a linear projection), and the fixed per-launch overhead compounds across all layers and all generated tokens. Production inference engines such as vLLM~\citep{vLLM}, SGLang~\citep{sglang}, and TensorRT-LLM mitigate this with fused \texttt{RMSNorm+linear} kernels that read the activation from HBM once and keep the intermediate in registers. These fused kernels assume the standard RMSNorm-then-project dataflow; a FlashNorm-folded checkpoint loads correctly but cannot exploit the fused fast path without engine-side changes. Bridging this gap, while preserving exact mathematical equivalence to the original model, is the practical problem we address.
...
\begin{itemize}
  \item \textbf{Exact reformulation.} We prove two commutativity results (Propositions~\ref{prop:weightless} and~\ref{prop:commute}) that characterize when weight folding and deferred scaling preserve the bias-free RMSNorm-then-project computation.
  \item \textbf{Unified coverage.} The same transformations extend to LayerNorm, Dynamic Tanh~\citep{DyT}, GLU-variant feed-forward networks~\citep{GLU}, and RoPE-based attention~\citep{RoPE}, including the QK-normalization case used by Gemma~3 and recent models.
  \item \textbf{Quality verification across model scales.} We report source-vs-folded deltas on SmolLM2-135M, Llama-3.2-1B, and Llama-3.1-8B across wikitext perplexity, MMLU 5-shot, and HellaSwag zero-shot (Table~\ref{tab:quality}), and verify bit-identical fp32 greedy generation at all three scales. We release three public compatibility checkpoints that load unmodified in stock HuggingFace Transformers and vLLM.
  \item \textbf{GPU evidence spanning three generations.} We report operation-level speedups on NVIDIA T4 (Turing), A100 (Ampere), and H100 (Hopper). The A100 evidence includes a fused Triton prototype benchmarked against three realistic baselines (Section~\ref{sec:a100-fused}) and a hand-rolled Sm80 kernel delivering end-to-end decode wins at Llama-family scales (Appendix~\ref{app:a100-decode}). The H100 evidence uses a CUTLASS 3.x Sm90 reference kernel benchmarked against HuggingFace and flashinfer~\citep{flashInfer} (the fused RMS kernel used internally by vLLM and SGLang), detailed in Appendix~\ref{app:h100-cutlass}.
  \item \textbf{Production integration path.} We quantify where the fusion pattern already wins at production scales, identify the specific kernel-engineering work (shape-specialized tile dispatch and custom \texttt{CollectiveMainloop}) required to extend the wins to Llama-family compute-bound regimes, and provide a reference design for that extension.
\end{itemize}
...
\paragraph{Scope: inference-time transformation.} FlashNorm is an inference-time transformation applied to an already-trained model. Folding $\mathbf{g}$ into $\mathbf{W}^\ast$ and deferring $1/\mathrm{RMS}(\mathbf{a})$ are both mathematically exact (Propositions~\ref{prop:weightless} and~\ref{prop:commute}), so the resulting model's outputs are bit-identical to the source under fp32 inference and within benchmark noise under fp16. The transformation does not modify the trained weights, the training process, or the loss landscape; all reported experiments therefore measure only inference quality and latency. Training-time application of the same reordering is possible in principle and is left to future work.
...
A CUTLASS 3.x Sm90 fused \texttt{RMSNorm+linear} kernel built on NVIDIA's production \texttt{CollectiveBuilder} and Epilogue Visitor Tree templates (Appendix~\ref{app:h100-cutlass}) confirms that the fusion pattern transfers to production-grade Sm90 infrastructure. Against the HuggingFace baseline the prototype wins on all 18 shapes; against the optimized flashinfer baseline it wins on 10 of 18 (all SmolLM2 and Llama-3.2-1B decode shapes), with the remaining 8 losses attributable to a single non-specialized tile choice in the prototype. Closing that gap requires shape-specialized tile dispatch and a custom \texttt{CollectiveMainloop} that fuses the RMS accumulation into the matmul's existing tile load, both routine kernel-team work inside production inference frameworks. Landing this kernel into vLLM~\citep{vLLM} is the natural next step. Further integration targets include
SGLang~\citep{sglang}, FlashInfer~\citep{flashInfer},
