...
\author{Nils Graef\thanks{\texttt{info@openmachine.ai}}, \, Andrew Wasielewski, \, Matthew Clapp, \, Filip Makraduli \\
...
To validate the parallel execution described in Fig.~\ref{fig8}, we benchmarked the norm-then-project operation on an NVIDIA T4 GPU (65~TFLOPS FP16, 320~GB/s). We compare sequential execution (RMSNorm $\rightarrow$ GEMM) against FlashNorm (GEMM $\|$ $1/\rms$ $\rightarrow$ scale), where the GEMM runs on tensor cores and $1/\rms$ runs on vector cores via a Triton kernel on separate CUDA streams. Table~\ref{tab:overlap} shows the results at SmolLM2-135M scale, and Table~\ref{tab:overlap7b} at Llama-7B scale.

\begin{table}[h!] \centering
\caption{Norm+project latency on T4 GPU at SmolLM2-135M scale ($n{=}576, k{=}960$).}
\label{tab:overlap}
\begin{tabular}{r r r r l}
\hline
Tokens & Sequential & FlashNorm & Speedup & Regime \\
\hline
1      & 0.374\,ms & 0.616\,ms & $-$64.8\% & decode (mem-bound) \\
64     & 0.506\,ms & 0.737\,ms & $-$45.8\% & decode (mem-bound) \\
4096   & 0.704\,ms & 0.468\,ms & $+$33.6\% & prefill (compute-bound) \\
8192   & 0.929\,ms & 0.599\,ms & $+$35.5\% & prefill (compute-bound) \\
\hline
\end{tabular}
\end{table}
...
\hline
Tokens & Sequential & FlashNorm & Speedup \\
\hline
1      & 0.641\,ms & 0.927\,ms & $-$44.6\% \\
64     & 0.307\,ms & 0.644\,ms & $-$109.8\% \\
256    & 0.577\,ms & 0.546\,ms & $+$5.5\% \\
1024   & 1.882\,ms & 1.654\,ms & $+$12.1\% \\
4096   & 7.628\,ms & 6.570\,ms & $+$13.9\% \\
\hline
\end{tabular}
\end{table}

The speedup appears only when the GEMM is compute-bound (many tokens, i.e.\ prefill). For memory-bound GEMMs (few tokens, i.e.\ decode), stream management overhead dominates. Profiling individual operations within a SmolLM2-135M decoder layer at 4096 tokens confirms that the $1/\rms$ calculation takes ${\sim}0.08$\,ms per layer. Across 30 layers, the theoretical maximum savings from perfect overlap is ${\sim}5$\,ms out of ${\sim}60$\,ms total (${\sim}8\%$).
