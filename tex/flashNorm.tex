...
% FlashNorm is mathematically identical to the original computation---it introduces
% no approximation and requires no retraining.
And due to RMS scale invariance, a linear layer between two RMSNorms makes the
%first one redundant---a mathematically identical simplification that removes the
...
% via PyTorch's existing weightless C++ kernel (Appendix~\ref{app:rmsnorm-fraction}).
...
\emph{non-parametric}~\citep{olmo2}. \citet{remove-norm2} likewise show that LayerNorm
can be removed post-hoc from GPT-2 XL with a learned ablation procedure that recovers
quality on language-modeling and downstream tasks. The two settings are complementary
to ours: OLMo's non-parametric form is a training-time architectural choice, and the
post-hoc removal in \citet{remove-norm2} is an empirically tuned procedure that may
introduce small quality deltas. FlashNorm contributes a closed-form algebraic
restatement that is exact under real arithmetic and bit-identical when the fold is
performed at the runtime precision, applies to any pretrained RMSNorm checkpoint
without retraining or empirical recovery, and additionally exposes a parallel-execution
opportunity (Proposition~\ref{prop:commute}) that the prior weightless / removal lines
do not address.
...
\]
where $\mathbf{g} \in \mathbb{R}^n$ are learned normalization weights. Production implementations include a small regularizer inside the square root, $\mathrm{RMS}_\varepsilon(\mathbf{a}) = \sqrt{\varepsilon + \tfrac{1}{n}\sum_i a_i^2}$, to prevent division by zero; we use the unregularized form for the proofs of Propositions~\ref{prop:weightless} and~\ref{prop:commute}, and address the $\varepsilon$-regularized case in Section~\ref{sec:rms-cancel} and Appendix~\ref{app:epsilon}.
...
Deferring the scalar $1/\mathrm{RMS}(\mathbf{a})$ to after the matrix multiplication alters the dynamic range of the intermediate values: $\mathbf{a}\mathbf{W}^*$ can be substantially larger than $(\mathbf{a}/\mathrm{RMS}(\mathbf{a}))\mathbf{W}^*$ for outlier-prone activations. Implementations therefore accumulate this intermediate in a higher precision than the storage format (typically FP32 accumulators behind FP16/BF16 storage); see Section~\ref{sec:limitations-discussion} for the dynamic-range discussion and the FP8 caveat.
\end{remark}
...
% before:
%The commutativity in Proposition~\ref{prop:commute} depends only on $1/\mathrm{RMS}(\mathbf{a})$ being a scalar per token; it holds identically if $\mathbf{W}^*$ is replaced by the unfolded $\mathbf{W}$. Deferred normalization can therefore be applied at runtime to any RMSNorm-based model without modifying its checkpoint. Propositions~\ref{prop:weightless} and~\ref{prop:commute} are thus independent transformations that can be adopted separately or together: a framework that implements deferred normalization alone delivers the parallel-execution and fused-kernel speedups (Corollary~\ref{cor:parallel} and Appendix~\ref{app:a100}) to every existing RMSNorm-based checkpoint, without requiring a weight-folded variant.
\end{remark}
...
\end{proof}
%Thus, whenever a weightless RMSNorm is immediately followed by a bias-free linear layer and then another RMSNorm, the first normalization is redundant and can be removed as illustrated in Fig.~\ref{fig6}(b).
...
%Multi-head Latent Attention (MLA) compresses querries, keys, and values into a low-rank
%latent space that is normalized before being projected back to full dimension.
...
linear layer with weights $\mathbf{V} \in \mathbb{R}^{m \times n}$ (the algebra below is
written for the general rectangular case; the square case $m = n$ is the special case
used in many transformer sublayers), the centering
...
entirely, see Fig. \ref{figB}(b). This provides a recipe for converting a LayerNorm model
to RMSNorm without retraining; for transformers with a residual stream, the same transformation must be applied to every projection that injects into the residual stream (token embeddings, attention output projection, FFN down-projection, and any other architecture-specific contributors) so that the input to each LayerNorm is exactly zero-mean.
\begin{figure}[h!] \centering  % the [h!] tries to place the picture right here
...
relative to (a) as shown in Fig. \ref{fig3}(b) (the savings are with respect to the baseline of scaling at both $f$-dimensional Gate and Up outputs in (a); the absolute multiply count of (b) is $f + n$, vs $2f$ in (a)). See appendix for FFNs with ReLU, ReGLU and Bilinear GLU variants.
\begin{figure}[h!] \centering
...
interleaved layout.
% More details: $\mathrm{permute}(\mathbf{g}) = (g_2, g_1, g_4, g_3, \ldots, g_h, g_{h-1})$.
...
With row-major ordering, once the RMS accumulation completes and the scalar $1/\mathrm{RMS}(\mathbf{a})$ becomes available, the first rows of $\mathbf{W}^*$ can be processed while later elements are still being scaled by that scalar. This overlap of the matrix unit with the element-wise scaling reduces the bubble
...
\begin{figure}[h!] \centering
  ...
  \caption{Exemplary timing diagrams.
    ...
    %; 17 cycles total, a 29\% reduction over (a).}
...
a technique already used in practice (e.g., SmoothQuant~\citep{smoothQuant}). The fold $\mathbf{W}^*_{i,j} = g_i \cdot \mathbf{W}_{i,j}$ scales each row of $\mathbf{W}$ by the corresponding $g_i$: when $\mathbf{g}$ has small variation across channels, $\mathbf{W}^*$ has similar dynamic range to $\mathbf{W}$ and existing per-channel quantization recipes carry over unchanged; when $\mathbf{g}$ has wide variation, $\mathbf{W}^*$ has rows of disparate magnitudes, making per-channel rather than per-tensor quantization the natural target. The composition is in the same family as SmoothQuant's per-channel-scale absorption, with the practical difference that FlashNorm's $\mathbf{g}$ is the existing trained RMSNorm weight rather than an introduced calibration scale. We leave a systematic empirical study of this interaction (perplexity, MMLU, HellaSwag, and bits-per-weight across int8, int4, and NF4 with the fold applied before vs.\ after quantization) to future work.
...
The majority of contemporary frameworks employ floating-point activations alongside
either fixed-point or floating-point weights, and at FP16/BF16 our production kernels
accumulate the unnormalized matmul output in FP32, which mitigates this risk in practice.
Low-precision floating-point formats remain a separate concern: FP8 in the E4M3 layout has
a maximum representable value of approximately 448, so for outlier-prone activations the
unnormalized intermediate $\mathbf{a}\mathbf{W}^\ast$ can exceed the FP8 range even when the
normalized form would not. FP8 deployments therefore require either a higher-precision
accumulator or per-channel scaling of $\mathbf{W}^\ast$; we leave a systematic FP8 study
to future work.
...
\paragraph{Numerical sensitivity at larger scales.}
Under fp32 inference, post-hoc weight folding produces strictly identical greedy
generation across all three model scales. Under lossy inference precisions
(fp16/bf16 in HuggingFace Transformers at Llama-7B scale, or vLLM's
PagedAttention at any precision), precomputed merged weights can produce a
one-token argmax divergence at tight decision points, which downstream greedy
decoding then amplifies; the effect is more visible at larger model scales
where argmax decisions are tighter. This is a general consequence of
precomputing weight-folded tensors for lossy-inference kernels and is not
specific to FlashNorm. A native fused $\mathrm{RMSNorm}+\mathrm{QKV}$ kernel
that defers $\mathbf{g}$ to runtime, rather than precomputing it into
$\mathbf{W}^\ast$, eliminates the framework dependency. See Appendix~\ref{app:additional-notes} for further detail.
...
%The technique extends to LayerNorm, DyT, GLU-FFNs, and RoPE attention.
GPU benchmarks confirm 33--35\% operation-level latency reduction at the SmolLM2-135M scale and 12--14\% at the Llama-7B scale
...
\paragraph{Bounding the epsilon-induced scale-invariance discrepancy.}
The deferred-normalization identity (Proposition~\ref{prop:commute}) relies on the scale-invariance $\mathrm{RMS}(s\cdot\mathbf{a}) = s\cdot\mathrm{RMS}(\mathbf{a})$, which holds exactly for the unregularized RMS but not for $\mathrm{RMSe}(\mathbf{a}) = \sqrt{\varepsilon + M(\mathbf{a})}$ with $M(\mathbf{a}) = \tfrac{1}{n}\sum_i a_i^2$. We bound the discrepancy here to support the claim that it is negligible in practice.

For a scalar $c > 0$, $\mathrm{RMSe}(c\cdot\mathbf{a}) = \sqrt{\varepsilon + c^2 M(\mathbf{a})}$ whereas $c\cdot\mathrm{RMSe}(\mathbf{a}) = \sqrt{c^2 \varepsilon + c^2 M(\mathbf{a})}$. The relative error of one against the other is
\begin{equation*}
  \left| \frac{\mathrm{RMSe}(c\cdot\mathbf{a})}{c\cdot\mathrm{RMSe}(\mathbf{a})} - 1 \right|
  = \left| \sqrt{\frac{\varepsilon + c^2 M}{c^2 (\varepsilon + M)}} - 1 \right|
  \leq \frac{1}{2} \cdot \frac{|1 - c^2|\,\varepsilon}{c^2 (M + \varepsilon)}
\end{equation*}
to first order in $\varepsilon$ (using $\sqrt{1+x} - 1 \approx x/2$ for small $x$). For typical RMSNorm with $\varepsilon = 10^{-6}$, activations satisfying $M \approx 1$, and $c$ in a moderate range (say $\tfrac{1}{2} \leq c \leq 2$), the relative error is bounded by approximately $\tfrac{3}{2}\varepsilon \approx 1.5 \times 10^{-6}$, well below fp16 representation precision. The bound degrades only when the activation energy is near zero ($M \ll \varepsilon$); in that regime the epsilon's role is to bound the magnitude of the scaling factor rather than to preserve scale-invariance, and Proposition~\ref{prop:commute} ceases to be the relevant identity.
...
\paragraph{Findings.} RMSNorm occupies 24.62\% of decode wall-clock at bf16, an order of magnitude above its FLOP share, because the model runs 33 separate RMSNorm kernels per forward pass and each incurs a fixed kernel-launch latency on the order of $10\,\mu$s plus a memory-bandwidth pass over the full activation tensor independent of the small RMS arithmetic. Roughly half of this fraction is recoverable today via a runtime swap of \texttt{LlamaRMSNorm.forward}: on the same hardware, same decode protocol, replacing it with PyTorch's existing weightless C++ kernel \texttt{F.rms\_norm(x, shape, weight=None, eps)} yields a $\mathbf{+12.77\%}$ end-to-end speedup on the FlashNorm-folded variant \texttt{open-machine/Llama-3.2-1B-FlashNorm} ($50.75$ vs $45.00$\,tokens/s)\footnote{\url{https://github.com/OpenMachine-ai/transformer-tricks/blob/main/notebooks/flashNorm\_hf\_a100.ipynb}}. The $45.00$\,tokens/s baseline here is the FlashNorm-folded checkpoint executed under the default \texttt{LlamaRMSNorm.forward} (which still performs the per-channel multiply against the all-ones tensor that folding leaves behind); it differs from the $38.5$\,tokens/s baseline in Table~\ref{tab:rmsnorm-fraction}, which is the unfolded \texttt{unsloth/Llama-3.2-1B-Instruct} checkpoint measured in a separate Colab session for the cumulative-norm-fraction analysis. The $+12.77\%$ measurement is consequently a same-checkpoint, same-session A-vs-B comparison and isolates the contribution of the kernel-side dispatch swap from any checkpoint or session variability. The same measurement combines two independent contributions: (i) elision of the per-channel multiply (FlashNorm Proposition~\ref{prop:weightless}), and (ii) kernel fusion (replacing the multi-kernel manual variance-plus-rsqrt-plus-multiply path of \texttt{LlamaRMSNorm.forward} with a single fused C++ kernel call). The fusion contribution is independently established by public benchmarks of PyTorch's fused \texttt{torch.nn.functional.rms\_norm} kernel against the unfused PyTorch path on equivalent shapes; the multiply-elision contribution is independently established by kernel-isolated benchmarks of weightless vs. weighted RMSNorm kernels (e.g., the concurrent third-party PyPTX measurement at ${\sim}1.05\times$ on H100 fp32\footnote{\url{https://github.com/patrick-toulme/pyptx/issues/2}}). The two-source decomposition that explains the magnitude of our $+12.77\%$ end-to-end measurement is therefore consistent with the literature, with multiply elision the minority component and kernel fusion the majority.
The gain is platform-specific: the same A-vs-B swap on Apple Silicon (MPS, fp16, \texttt{open-machine/Llama-3.2-1B-FlashNorm}, 64-token greedy decode, median of 5 trials) yields $-28.34\%$ ($24.62$ vs $34.35$\,tokens/s), because PyTorch's MPS \texttt{F.rms\_norm} path lacks the vendor-tuned fast path the CUDA C++ kernel has, so the multi-kernel manual chain is faster on that platform. The realized end-to-end gain therefore depends on whether the framework's weightless path is itself well-optimized on the target hardware; the FlashNorm transformation is exact on every platform, but the realized speedup follows the kernel-quality landscape. The cumulative norm wall-clock is invariant under weight quantization (within $\pm 2\%$ across all three precisions: $1{,}640$, $1{,}675$, $1{,}650$\,ms), as BitsAndBytes does not quantize norm weights. The norm fraction is non-monotonic across precisions on this hardware ($24.62\% \to 8.19\% \to 9.31\%$) because BitsAndBytes inflates the linear path's wall-clock at low bit on A100 (\texttt{MatMul8bitLt} dequantize overhead makes 8-bit $3\times$ slower than bf16; NF4 is more efficient but still $2.7\times$ slower); the fraction shrinks at lower bit not because the norm becomes cheaper but because the BitsAndBytes linear becomes more expensive. On hardware-and-framework combinations with native low-bit kernels (e.g., \texttt{llama.cpp} GGUF, MLX 4-bit on Apple silicon, vLLM and TensorRT-LLM with int4 at scale), the invariant norm cost would translate to a monotonically growing fraction; direct measurement on those stacks is left to follow-up.
