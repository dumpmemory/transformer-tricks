...
On the FlashNorm-folded checkpoint \texttt{open-machine/Llama-3.2-1B-FlashNorm} at bf16 (NVIDIA A100, 256-token greedy decode, median of 20 trials), routing \texttt{LlamaRMSNorm.forward} through PyTorch's torch C++ kernel \texttt{F.rms\_norm(x, shape, weight=None, eps)}, which skips the per-channel multiply when the weight argument is \texttt{None}, yields $50.75$\,tokens/s versus $45.00$\,tokens/s for the default forward that always computes \texttt{self.weight * x\_normalized}: a relative speedup of $\mathbf{+12.77\%}$, recovering roughly half of the $24.6\%$ RMSNorm wall-clock ceiling reported in Appendix~\ref{app:rmsnorm-fraction}. Both variants run on the same checkpoint, isolating kernel choice from folding. The mechanism is Proposition~\ref{prop:weightless} composed with PyTorch's existing weightless code path. Reproducibility notebook in the \texttt{transformer-tricks} repository\footnote{\url{https://github.com/OpenMachine-ai/transformer-tricks/blob/main/notebooks/flashNorm\_hf\_a100.ipynb}}.
